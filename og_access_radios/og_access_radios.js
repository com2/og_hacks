...
Drupal.behaviors.og_access_group_post = function() {
  ...
  /* Node authoring form for group content -Disable the public checkbox if no groups are selected in in Audience */
    $('.og-audience').click(function() {
      ...
      if (cnt > 0 ) {
        $('input:radio[@name=og_public]').removeAttr('disabled');
		if (typeof Drupal.settings.og_public_default == "undefined"){
			$('input:radio[@name=og_public]').removeAttr('checked');
	  } else if (Drupal.settings.og_public_default[0] == 0){
		  $('input:radio#edit-og-public-0').attr("checked", "checked");
		} else if (Drupal.settings.og_public_default[0] == 1){
			$('input:radio#edit-og-public-1').attr("checked", "checked");
		  }
	  } else {
        $('input:radio[@name=og_public]').attr("disabled", "disabled");
		$('input:radio#edit-og-public-1').attr("checked", "checked");
      }
    });
